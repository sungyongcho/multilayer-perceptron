...
class NeuralNetwork:
    ...
    def init_weights(self, layer_index):
        ...
        if next_layer.weights_initializer == "heUniform":
            weights_matrix = heUniform_(begin_layer_nodes_count, next_layer_nodes_count)
            ...
            weights_matrix = np.zeros((begin_layer_nodes_count, next_layer_nodes_count))
    ...
    def init_biases(self, layer_index):
        ...
        if next_layer.weights_initializer == "heUniform":
            ...
            bias_vector = np.random.normal(
                0.0, pow(next_layer_nodes_count, -0.5), (next_layer_nodes_count, 1)
            )
        elif next_layer.weights_initializer == "zero":
            bias_vector = np.zeros((next_layer_nodes_count, 1))
        ...
        for i in range(num_layers - 2, -1, -1):
            ...
            if i > 0:
                weights_gradient = np.dot(
                    output_gradient * activation_gradient, self.outputs[i - 1].T
                ).T
            else:
                weights_gradient = np.dot(
                    output_gradient * activation_gradient, self.data.T
                ).T
    ...
    def fit(self, data_train, epoch_num, loss):
        ...
        for epoch in range(epoch_num):
            ...
            print(y_pred_batch)
            loss_2 = binary_crossentropy(y_train_binary, y_pred_batch)
            print(loss_2)
